...
void SignalMoleculeDiffusionModel::initCells(::std::vector<Cell*>& cells)
{
    ...
    if (fieldRadiusX == fieldRadiusY) [[likely]] {
        for (int32_t i = 0; i != SimulationSettings::CELL_NUM; i++) {
            ...
            cells.push_back(new SignalDiffusionCell(CellType::WORKER, r * ::cos(theta), r * ::sin(theta), 10.0));
        }
...
void SignalMoleculeDiffusionModel::onNextStep(::std::vector<Cell*>& cells, ::std::vector<MoleculeSpace*>& moleculeSpaces)
{
    SignalDiffusionMoleculeSpace* pSpace = static_cast<SignalDiffusionMoleculeSpace*>(moleculeSpaces[0]);
    for (Cell* p : cells) {
