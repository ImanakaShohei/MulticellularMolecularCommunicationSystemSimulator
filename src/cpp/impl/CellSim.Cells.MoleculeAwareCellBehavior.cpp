...
namespace CellSim::Cells
{
        ...
        , m_degradationRate(degradationRate)
    {
        if (synthesisRate <= 0.0) [[unlikely]] {
            throw ::std::runtime_error(
                Messages::Get(
                    "Cells.MoleculeAwareCellBehavior.MoleculeAwareCellBehavior.Error.synthesisRate"
                )
            );
        }
        if (degradationRate <= 0.0) [[unlikely]] {
            throw ::std::runtime_error(
                Messages::Get(
                    "Cells.MoleculeAwareCellBehavior.MoleculeAwareCellBehavior.Error.degradationRate"
                )
            );
        }
    }

    MolecularProcessResult MoleculeAwareCellBehavior::ComputeMolecularProcess(
        const Cell*,
    ...
    )
    {
        ...
        result.IntracellularChange = 
            (m_synthesisRate - m_degradationRate * args.IntracellularAmount) *
            Settings::Config::Simulation::DeltaTime();

        return result;
    }

    Numerics::Vector3 MoleculeAwareCellBehavior::OnSenseMolecules(
        const Cell*,
