...
namespace CellSim::Molecular
{
    ...
    MoleculeKind MoleculeKind::AddName(
        ::std::string_view name,
        double threshold
    )
    {
        for (size_t i = 0; i < s_names.size(); i++) {
            if (s_names[i].first == name) [[unlikely]] {
        ...
        return MoleculeKind(static_cast<uint32_t>(s_names.size() - 1));
    }
    ...
    MoleculeKind MoleculeKind::FromName(
        ::std::string_view name
    ) noexcept
    {
        for (size_t i = 0; i < s_names.size(); i++) {
            if (s_names[i].first == name) return MoleculeKind(static_cast<uint32_t>(i));
        }
